...
function run() {
    return __awaiter(this, void 0, void 0, function () {
        var referenceImage, queryImage, resultsRef, resultsQuery, faceMatcher, labels, refDrawBoxes, outRef, queryDrawBoxes, outQuery;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 7:
                    ...
                    refDrawBoxes = resultsRef
                        .map(function (res) { return res.detection.box; })
                        .map(function (box, i) { return new faceapi.draw.DrawBox(box, { label: labels[i] }); });
                    outRef = faceapi.createCanvasFromMedia(referenceImage);
                    refDrawBoxes.forEach(function (drawBox) { return drawBox.draw(outRef); });
                    commons_1.saveFile('referenceImage.jpg', outRef.toBuffer('image/jpeg'));
                    queryDrawBoxes = resultsQuery.map(function (res) {
                        var bestMatch = faceMatcher.findBestMatch(res.descriptor);
                        return new faceapi.draw.DrawBox(res.detection.box, { label: bestMatch.toString() });
                    });
                    outQuery = faceapi.createCanvasFromMedia(queryImage);
                    queryDrawBoxes.forEach(function (drawBox) { return drawBox.draw(outQuery); });
